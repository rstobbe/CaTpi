...
function [TST,err] = DesTest_CaTpiGslew_v1c_Func(TST,INPUT)
...
if strcmp(func,'TestPlot')
    ...
    plot(r,magvel0/gamma,'k-'); 
    if isfield(PROJdgn,'ConstCones')
        if PROJdgn.ConstCones ~= 0
            plot(r,magvel0(PROJdgn.ConstCones,:)/gamma,'b-');
            plot(r,magvel0(PROJdgn.ConstCones(1),:)/gamma,'r-');
    ...
    if isfield(PROJdgn,'ConstCones')
        if PROJdgn.ConstCones ~= 0
            plot(r,magacc0(PROJdgn.ConstCones,:)/gamma,'b-');
            plot(r,magacc0(PROJdgn.ConstCones(1),:)/gamma,'r-');
    ...
    plot(r,magjerk0/gamma,'k-');
    if isfield(PROJdgn,'ConstCones')
        if PROJdgn.ConstCones ~= 0
            plot(r,magjerk0(PROJdgn.ConstCones,:)/gamma,'b-');
            plot(r,magjerk0(PROJdgn.ConstCones(1),:)/gamma,'r-');
    ...
    plot(DESMETH.TPIT.CaTpiTimeAtRad,DESMETH.TPIT.Rad,'k-');
...
Panel0(7,:) = {'Ncones',PROJdgn.ncones,'Output'};
Panel0(8,:) = {'p',PROJdgn.p,'Output'};
Panel0(9,:) = {'ProjOverSamp',PROJdgn.projosamp,'Output'};
Panel0(10,:) = {'EdgeSampDens',PROJdgn.edgeSD,'Output'};
