...
function [DESOL,err] = DeSolTim_TpiManual_v1f_Func(DESOL,INPUT)
...
while true    
    if rendval > 0.002
        xnew = xprev * (1 + 0.0001*rendval);
    elseif rendval > 0.001
        xnew = xprev * (1 + 0.00003*rendval);
    elseif rendval > 0.0005
        xnew = xprev * (1 + 0.00001*rendval);
    elseif rendval > 0.00025
        xnew = xprev * (1 + 0.000003*rendval);
    elseif rendval > 0.0001
        xnew = xprev * (1 + 0.000001*rendval);
    else 
        xnew = xprev * (1 + 0.0000003*rendval);
    end
    xprev = xnew;
    tau = [(0:-1e-6:x(ind-1)) xnew];
    tol = 2.5e-14;
    options = odeset('RelTol',tol,'AbsTol',tol);
    [x,r] = ode113(defunc,tau,p,options);  
    ...
    tauout = tau;
    n = n+1;
    Status2('busy',['Error:  ',num2str(rendval)],3);
end
