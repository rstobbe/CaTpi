...
function [PSMP,err] = ProjSamp_Cones_v1g_Func(PSMP,INPUT)
...
%---------------------------------------------
thnproj = ceil((4*pi*(PROJdgn.rad)^2)*PROJdgn.p);
sampling = PROJdgn.nproj/thnproj;
if sampling < 1.05
    phithetafrac0 = 1;
else
    phithetafrac0 = 0.2;
...
%----------------------------------------------------
% No Extra Projections on Constrained Cones
%----------------------------------------------------
eproj = 0;
...
INPUT.phithetafrac0 = phithetafrac0;
INPUT.eproj = eproj;
