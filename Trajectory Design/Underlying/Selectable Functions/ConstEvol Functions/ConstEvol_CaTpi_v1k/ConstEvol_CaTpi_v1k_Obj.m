...
classdef ConstEvol_CaTpi_v1k_Obj < handle
...
% Constructor
%---------------------------------------------
function obj = ConstEvol_CaTpi_v1k_Obj(kArr,tArr,GradSlewMax,GradAccMax,PROJdgn,PROJimp,TST)
    obj.kArr = kArr;
    obj.tArr = tArr;
...
function bool = TestRadGreaterP(obj)
    bool = 0;
    rad = obj.kMagArr/obj.kMagArr(end);
    if rad(obj.Loc) > obj.PROJdgn.p
        bool = 1;
    end
end
function bool = TestGreaterRelRad(obj,Val)
    bool = 0;
    rad = obj.kMagArr/obj.kMagArr(end);
    if rad(obj.Loc) > Val
        bool = 1;
...
function EstimateInitVelAcc(obj)
    for n = 2:length(obj.kArr)
        DistVec = obj.kArr(n,:)-obj.kArr(n-1,:);
        ...
        obj.Jrk0(n) = (obj.Acc0(n)-obj.Acc0(n-1))/tSeg;
    end
end  
...
function tSeg = AdjustSegSpecifyAcc(obj,AccVal,tSeg0)    
    ...
    while true
        if AccVal < 1e-5
            ...
            if abs(Acc0) < 1.00001*abs(AccVal) && abs(Acc0) > 0.99999*abs(AccVal)
            %if abs(Acc0) < 1.000001*abs(AccVal) && abs(Acc0) > 0.999999*abs(AccVal)
                tSeg = tSeg0;
                break
            end
        end
        ...
        its = its + 1;
        if its > 50
            if abs(Acc0) < 1.001*abs(AccVal) && abs(Acc0) > 0.999*abs(AccVal)
                tSeg = tSeg0;
                break
            else
                error
...
function PlotEvolution(obj,num)   
    if rem(obj.Loc,num) == 0 
        ...
        subplot(2,3,4); 
        obj.PlotEvLineHands(6) = plot(obj.tSamp(1:obj.Loc-1),obj.tSegArray(1:obj.Loc-1)*1000,'k');
        subplot(2,3,5); 
