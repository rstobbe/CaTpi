...
function [IMETH,err] = ImpMeth_CaTpi_v1d_Func(IMETH,INPUT)
...
if strcmp(TST.TVis,'Yes')
    ...
    plot3(PROJdgn.rad*KSA(1,:,1),PROJdgn.rad*KSA(1,:,2),PROJdgn.rad*KSA(1,:,3)*PROJdgn.elip,'k-');
    lim = PROJdgn.rad;
    xlim([-lim,lim]); ylim([-lim,lim]); zlim([-lim*PROJdgn.elip,lim*PROJdgn.elip]);
    ...
    plot3(PROJdgn.rad*KSA(1,1:ind,1),PROJdgn.rad*KSA(1,1:ind,2),PROJdgn.rad*KSA(1,1:ind,3)*PROJdgn.elip,'k-');
    ploc = interp1(Rad,PROJdgn.rad*squeeze(KSA(1,:,:)),PROJdgn.p,'spline');
    plot3(ploc(1),ploc(2),ploc(3)*PROJdgn.elip,'rx');
...
INPUT.TST = TST;
INPUT.FinalSolution = 0;
if strcmp(IMETH.eliploc,'BeforeConst')
    INPUT.DoElip = 1;
end
[OUTPUT,err] = genprojfunc(INPUT);
if err.flag
...
%--------
% Test.tArr = T;
% Test.kArr = squeeze(KSA(maxtraj,:,:));
% Test.PROJdgn = PROJdgn;
% save('Test','Test');
% error
%--------
...
% if round(testtro*1e2) ~= round(PROJdgn.tro*1e2)
%     error
% end
...
%---------------------------------------------
% Find 'hardest' trajectory (for display)
%---------------------------------------------
[vel,Tvel0] = CalcVelMulti_v2a(KSA*PROJdgn.kmax,T);
[acc,Tacc0] = CalcAccMulti_v2a(vel,Tvel0);
magacc0 = sqrt(acc(:,:,1).^2 + acc(:,:,2).^2 + acc(:,:,3).^2);
...
if strcmp(TST.GVis,'Yes')
    ...
    plot(L,Gvis(maxtraj,:,1),'b:'); plot(L,Gvis(maxtraj,:,2),'g:'); plot(L,Gvis(maxtraj,:,3),'r:');
    title(['Traj ',num2str(maxtraj)]);
...
maxcartgstepssmooth = smooth(maxcartgsteps,7);
GWFM.Gcompmaxcartslewsmooth = max(maxcartgstepssmooth)/GQNT.gseg;
for p = 1:length(cartg2drv(1,:,1))
    maxcartg2drvT(p) = max(max(abs(cartg2drv(:,p,:))));
end
...
if strcmp(TST.GVis,'Yes') 
    ...
    sp4 = plot(qTcomp(2:length(qTcomp)-2),maxcartgstepssmooth/GQNT.gseg,'b-');
    sp5 = plot(qTcomp(2:length(qTcomp)-2),maxmaggsteps/GQNT.gseg,'k:');
    title('Max Gradient Speed');
    ylabel('Gradient Speed (mT/m/ms)','fontsize',10,'fontweight','bold');
    xlabel('(ms)','fontsize',10,'fontweight','bold');
    legend([sp3,sp4,sp5],'Max Channel Speed','Max Channel Speed Smooth','Mean Total Speed','Location','southwest');
...
GwfmPanel(3,:) = {'GmaxChanSlewSmooth (amp)',GWFM.Gcompmaxcartslewsmooth,'Output'};
GwfmPanel(4,:) = {'GmaxChan2Drv (amp)',GWFM.Gcompmaxcart2drv,'Output'};
GwfmPanel(5,:) = {'kRadAt1ms (1/m)',GWFM.kRadAt1ms,'Output'};
GwfmPanel(6,:) = {'kRadAt03ms (1/m)',GWFM.kRadAt03ms,'Output'};
...
if strcmp(TST.KVis,'Yes') 
    ...
    p1 = plot(Samp0,Kmat0(maxtraj,:,1),'k-'); plot(Samp0,Kmat0(maxtraj,:,2),'k-'); plot(Samp0,Kmat0(maxtraj,:,3),'k-');
    p2 = plot(SampRecon,KmatRecon(maxtraj,:,1),'b-'); plot(SampRecon,KmatRecon(maxtraj,:,2),'g-'); plot(SampRecon,KmatRecon(maxtraj,:,3),'r-');
    p3 = plot(qT,GQKSA(maxtraj,:,1),'b*'); plot(qT,GQKSA(maxtraj,:,2),'g*'); plot(qT,GQKSA(maxtraj,:,3),'r*');
...
%---------------------------------------------
% Find p location (in Time)
%---------------------------------------------
rRad2 = mean(Rad/PROJdgn.kmax,1);
PROJimp.pLocTime = interp1(rRad2,samp-samp(1),PROJdgn.p);
...
KsmpPanel(6,:) = {'Sampling Delay (us)',KSMP.StartTimeOnWfm,'Output'};
...
plot3(Kmat(maxtraj,:,1),Kmat(maxtraj,:,2),Kmat(maxtraj,:,3),'k-');
lim = PROJdgn.kmax;
xlim([-lim,lim]); ylim([-lim,lim]); zlim([-lim*PROJdgn.elip,lim*PROJdgn.elip]);
xlabel('k_x (1/m)'); ylabel('k_y (1/m)'); zlabel('k_z (1/m)'); title('Final Trajectory');

subplot(1,3,2); hold on;
plot(samp,Kmat(maxtraj,:,1),'b');
plot(samp,Kmat(maxtraj,:,2),'g');
plot(samp,Kmat(maxtraj,:,3),'r');
plot([0 PROJdgn.tro],[0 0],':');
xlim([0,PROJdgn.tro]);
ylim([-lim*PROJdgn.elip,lim*PROJdgn.elip]);
xlabel('time (ms)');
ylabel('k (1/m)');
...
plot(L,Gvis(maxtraj,:,1),'b');
plot(L,Gvis(maxtraj,:,2),'g');
plot(L,Gvis(maxtraj,:,3),'r');
plot(L,-Gvis(end,:,3),'k');
...
IMETH.KINFO = func('');
IMETH.KINFO.SetFov(PROJdgn.fov);
IMETH.KINFO.SetVox(PROJdgn.vox);
IMETH.KINFO.SetDesignTro(PROJdgn.tro);
IMETH.KINFO.SetTro(PROJdgn.tro);
IMETH.KINFO.SetNproj(PROJdgn.nproj);
IMETH.KINFO.SetDesignSamplingTimeStart(0);
IMETH.KINFO.SetSamplingPtStart(KSMP.DiscardStart+1);
IMETH.KINFO.SetSamplingPts(KSMP.nproRecon);
IMETH.KINFO.SetSamplingTimeOnTrajectory(samp);
IMETH.KINFO.SetElip(PROJdgn.elip);
IMETH.KINFO.SetkSpace(Kmat);
IMETH.KINFO.SetDimx(ORNT.dimx);
IMETH.KINFO.SetDimy(ORNT.dimy);
IMETH.KINFO.SetDimz(ORNT.dimz);
IMETH.KINFO.SetSamplingPtAtCentre(1);
IMETH.KINFO.SetSamplingTimeToCentre(samp(1));
